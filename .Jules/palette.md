...
## 2024-05-18 - Mathematical Symbol Accessibility
**Learning:** Screen readers often misinterpret or completely ignore standalone mathematical symbols (like `+`, `-`, `*`, `/`, `=`) used in interactive components like calculators, leading to a confusing experience for visually impaired users.
**Action:** Explicitly define `aria-label` attributes for all standalone mathematical symbols and operator buttons so their function (e.g., "Add", "Subtract", "Multiply", "Divide", "Equals") is clearly announced.
