...
## 2026-05-28 - Decorative Timeline Images
**Learning:** To prevent redundant screen reader announcements, timeline illustrations accompanied by descriptive titles should be treated as purely decorative by setting `alt=""` and `aria-hidden="true"` on the image.
**Action:** Always verify if an image is purely decorative when accompanied by descriptive text, and apply `alt=""` and `aria-hidden="true"` if so.
